...
class AttnDecoderRNN(nn.Module):
    def __init__(self, attn_model, hidden_size, output_size, embedding_matrix, n_layers, dropout=0):#, max_length):
    ...
    def forward(self, input_seq, last_hidden, encoder_outputs):
        ...
        concat_input = torch.cat((rnn_output.squeeze(0), context.squeeze(1)), 1) # batch_size x 2*hidden_size
        concat_output = torch.tanh(self.concat(concat_input)) # batch_size x hidden_size
        # Finally predict next token (Luong eq. 6, without softmax)
        output = self.out(concat_output) # batch_size x vocab_output_size
