...
from evaluate_ppx import transform_from_index
...
def trainIters(data, val_data, 
    encoder, decoder, 
    input_voc, output_voc, 
    criterion, 
    n_epochs, batch_size, n_val, opt,
    ...
    while epoch < n_epochs:
        ...
        for step in range(1, len(data)//batch_size+1):
            ...
            loss = train(
                    input_batches[step-1], input_lengths[step-1], target_batches[step-1], target_lengths[step-1],
                    encoder, decoder,
        ...
        val_model_name = '%s%s_%d%s%d%s%d%s%d%s%d' % (
