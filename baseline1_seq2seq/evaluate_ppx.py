...
def evaluate(input_seq, input_lengths, tar_seq, tar_lengths, batch_size, encoder, decoder, input_voc, output_voc, max_length=MAX_LENGTH, USE_CUDA = True):
    '''
    ...
    for di in range(max_length):
        ...
        prob_mat = F.softmax(decoder_output.data, dim = 1) # batch_size x vocab_size
        for i in range(batch_size):
            topv[i]= prob_mat[i][tar_seq[:,i][di]].unsqueeze_(0)
        topv_p = topv # The target probability
        topi_p = topi # Output index
    ...
    return decoded_words, decoded_words_one_EOS, decoder_attentions[:di+1, :len(encoder_outputs)], ppx



def evaluate_and_show_attention(val_input, val_input_lengs, val_target, val_target_lengs, batch_size, encoder, decoder, input_voc, output_voc):


    output_words, decoded_words_one_EOS, attentions, ppx = evaluate(val_input, val_input_lengs, val_target, val_target_lengs, batch_size, encoder, decoder, input_voc, output_voc)
    ...
    return val_input, output_sentences, output_sentences_one_EOS, val_target, ppx


def evaluate_randomly_ppx(val_input, val_input_lengs, val_target, val_target_lengs, batch_size, encoder, decoder, input_voc, output_voc, target = True):
    if target == True:
        input_sentence, output_sentences, output_sentences_one_EOS, target_sentence, ppx = evaluate_and_show_attention(val_input, val_input_lengs, val_target, val_target_lengs,
            batch_size, encoder, decoder, input_voc, output_voc)
        return input_sentence, output_sentences, output_sentences_one_EOS, target_sentence, ppx
