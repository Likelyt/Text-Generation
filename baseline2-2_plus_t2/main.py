...
if __name__ == "__main__":
    ...
    if topic_choice == 'NTM':
        ...
        # Or load
        load_topic_name = '%s%s%s%s%s%d%s%s%s' % ('latent_topic_distribution/',opt.review_name,'_',opt.topic_choice,'_zh_',opt.topic_dimension,'_',len(data),'.txt')
    ...
    if topic_choice == 'LDA':
        ...
        optimal_epoch, val_model_name = trainIters(train_data, val_data, encoder, decoder, topic_choice,
                voc_input, voc_output, 
                criterion, opt.end_epoch, batch_size, n_sample,opt, TT_optimal_epoch=32,
                print_every=10, plot_every=1, learning_rate=lr, USE_CUDA=True)
