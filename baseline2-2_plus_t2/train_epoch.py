...
def trainIters(data, val_data, encoder, decoder, topic_choice,
                ...
                criterion, n_epochs, batch_size, n_val, opt, TT_optimal_epoch,
    ...
    checkpoint_path_TT =  '%s%d%s%d%s' %(
        'model_result/TT_rest_2241_TZ_', opt.topic_dimension,'_epoch_',TT_optimal_epoch, '.pt'
        )

    checkpoint_TT = load_checkpoint(checkpoint_path_TT)
    TT = checkpoint_TT['TT'].cuda()
    ...
    while epoch < n_epochs:
        ...
        val_model_name = '%s%s_%d%s%d%s%d%s%d%s%d%s%s%d' % (
