...
def evaluate(input_seq, input_lengths, tar_seq, tar_lengths, 
    ...
    for di in range(max_length):
        decoder_output, decoder_hidden, decoder_attention = decoder(decoder_input, decoder_hidden, val_input_topic, encoder_outputs)
        decoder_attentions[:,di,:decoder_attention.size(2)] += decoder_attention.squeeze(0).squeeze(0).squeeze(1).cpu().data
        topv, topi = decoder_output.data.topk(1)
        prob_mat = F.softmax(decoder_output.data, dim = 1) # batch_size x vocab_size
        for i in range(batch_size):
...
def evaluate_and_show_attention(val_input, val_input_lengs, val_target, val_target_lengs, val_input_topic, val_target_topic,
    ...
    output_words, decoded_words_one_EOS, attentions, ppx = evaluate(val_input, val_input_lengs, val_target, val_target_lengs,
        val_input_topic, val_target_topic, 
        batch_size, encoder, decoder, input_voc, output_voc)
...
def evaluate_randomly_ppx(val_input, val_input_lengs, val_target, val_target_lengs, 
    val_input_topic, val_target_topic,
