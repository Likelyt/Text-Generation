...
if __name__ == "__main__":
    ...
    '''
    data_pair = np.loadtxt('yelp-data/data_pair_24K.txt', delimiter=',', dtype = np.str).tolist()
    ...
    if topic_choice == 'NTM':
        ...
        ntm_batch_size = 128
        ntm_lr = 0.001
        sentence_topic_distribution = NTM_topic_generation('data_pair_24K', ntm_topic_dim, ntm_h_dim, ntm_num_epochs,  ntm_batch_size, ntm_lr).tolist()
        # Or load
        #sentence_topic_distribution = np.loadtxt('latent_topic_distribution/latent_z_24K.txt', delimiter=' ', dtype = 'float').tolist()
    ...
    decoder = AttnDecoderRNN(attn_model, hidden_size, voc_output.n_words, number_of_topic, n_layers_decoder, dropout)
    ...
    if topic_choice == 'LDA':
        trainIters(train_data, val_data, encoder, decoder, lda_topic, topic_choice,
                ...
                criterion, n_epochs, batch_size, n_sample, mlp_epoch,
                print_every=10, plot_every=1, learning_rate=lr, USE_CUDA=True)
    else:
        trainIters(train_data, val_data, encoder, decoder, ntm_topic, topic_choice,
                voc_input, voc_output, 
                criterion, n_epochs, batch_size, n_sample, mlp_epoch,
                print_every=10, plot_every=1, learning_rate=lr, USE_CUDA=True)
...
if __name__ == "__main__":
    ...
    np.savetxt('yelp-data/data_pair_100K.txt', data_pair, delimiter=',', fmt = '%s') 
    '''
    data_pair = np.loadtxt('yelp-data/rest_data_pair_24K.txt', delimiter=',', dtype = np.str).tolist()
    data, keep_index, train_encoder, train_decoder, voc_input, voc_output = create_voc_4.build_voc(data_pair)
