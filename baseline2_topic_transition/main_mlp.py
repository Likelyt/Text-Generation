...
from testing_mlp import testing_load_checkpoint_mlp
...
##Topic model options
parser.add_argument("-topic_dimension", type=int, default=200, help="Number of topics J")
...
parser.add_argument("-mlp_epoch",type=int, default=100, help="Epoch train for mlp")
...
if __name__ == "__main__":
    ...
    # Define Topic transition model
    USE_CUDA = True
    TT = Topic_Transition(opt.hidden_size+opt.topic_dimension, opt.hidden_size, opt.topic_dimension).cuda()


    optimal_epoch = 24
    # Load the upper bound model
    val_model_name = 'model_result/rest_2241_hidden_size_256_E_layer_2_D_layer_1_Enc_bidi_1'
    MLP_optimal_epoch = testing_load_checkpoint_mlp(optimal_epoch, train_data, val_data, test_data, opt.review_name, val_model_name, opt, TT, True)
