...
def NTM_topic_generation(data_name, z_dim, h_dim, num_epochs, batch_size, lr = 0.0001):
    # 0. Load Data Pairs
    data_pair = np.loadtxt('yelp-data/'+ data_name+'.txt', delimiter=',', dtype = np.str).tolist()
    ...
    np.savetxt('latent_topic_distribution/NTM_latent_z_24K.txt',latent_z_np_stack, fmt='%.4e', delimiter=' ')
