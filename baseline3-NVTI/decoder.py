...
class AttnDecoderRNN(nn.Module):
    def __init__(self, attn_model, hidden_size, output_size, num_topic, embedding_matrix, n_layers, dropout):#, max_length):
        ...
        self.gru = nn.GRU(self.embedding_size, hidden_size, n_layers, dropout=dropout)
    ...
    def forward(self, input_seq, last_hidden, target_topic, encoder_outputs):
        ...
        # embedded = self.embedding(input_seq)  # Batch_size x hidden_size
        embedded = self.embedding_pretrained(input_seq)
        embedded = self.embedding_dropout(embedded)
        # embedded = embedded.view(1, self.batch_size, self.hidden_size) # 1 x B x N
        ...
        # Choice 1: transform topic dimension into hidden size
        #if target_topic.size(1)!= self.hidden_size:    
        target_topic_output = self.topic_transofm(target_topic)
        #choice 2: just concatenate the topic vector into hidden, attention:
        #else:
        #target_topic_output = target_topic
