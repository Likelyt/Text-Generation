...
def evaluate(input_seq, input_lengths, 
    x,
    tar_seq, tar_lengths,
    batch_size, 
    ...
    TT.training = True
    return decoded_words, decoded_words_one_EOS, decoder_attentions[:di+1, :len(encoder_outputs)], PPX_all/(h1_val_samples*h2_val_samples)
...
def evaluate_and_show_attention(val_input, val_input_lengs, val_target, val_target_lengs, 
    ...
    output_words, decoded_words_one_EOS, attentions, ppx = evaluate(val_input, val_input_lengs,
        x,
        val_target, val_target_lengs, 
        batch_size, 
        encoder, decoder, model_vae_1, TT,
        input_voc, output_voc,
...
def evaluate_randomly_ppx(val_input, val_input_lengs, val_target, val_target_lengs,
    ...
    if target == True:
        input_sentence, output_sentences, output_sentences_one_EOS, target_sentence, ppx = evaluate_and_show_attention(
