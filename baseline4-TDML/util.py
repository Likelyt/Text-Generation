...
def gen_data(vocabxid, dummy_symbols, ignore, corpus, tm_sent_len, lm_sent_len, verbose, remove_short):
    ...
    for line_id, line in enumerate(codecs.open(corpus, "r", "utf-8")):
        ...
        for s, ss in [line.strip().split(",")]:
            sent = [vocabxid[start_symbol]]
            sent_2 = [vocabxid[start_symbol]]
            ...
            for w in ss.strip().split():
                if w in vocabxid:
                    sent_2.append(vocabxid[w])
                else:
                    sent_2.append(vocabxid[unk_symbol])
            sent_2.append(vocabxid[end_symbol])
            lm_sents.append(sent_2)
